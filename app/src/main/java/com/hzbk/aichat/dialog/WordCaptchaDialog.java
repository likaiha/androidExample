...
import com.hzbk.aichat.http.network.BaseObserver;
import com.hzbk.aichat.http.network.RetrofitUtils;
import com.hzbk.aichat.http.network.RxHelper;
import com.hzbk.aichat.util.AESUtil;
import com.hzbk.aichat.widget.WordImageView;
...
public class WordCaptchaDialog extends Dialog {
    ...
    private void loadCaptcha() {
        ...
        RetrofitUtils.getServerApi()
                .getWordCaptchaAsync(body)
                .compose(RxHelper.observableIO2Main(mContext))
                .subscribe(new BaseObserver<WordCaptchaGetIt>(mContext, true) {
            @Override
    ...
    private void checkCaptcha(String cryptedStr) {
        ...
        RequestBody body = RequestBody.create(MediaType.parse("application/json"), jsonObject.toString());
        Log.i("wuyan", "body:" + jsonObject);
        RetrofitUtils.getServerApi().checkAsync(body)
                .compose(RxHelper.observableIO2Main(mContext))
                .subscribe(new BaseObserver<CaptchaCheckIt>(mContext, false) {
            @Override
