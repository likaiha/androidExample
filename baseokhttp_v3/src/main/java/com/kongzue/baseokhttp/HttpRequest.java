...
import com.kongzue.baseokhttp.util.SSLSocketClient;
...
public class HttpRequest extends BaseOkHttp {
    ...
    public static void PATCH(Context context, String url, Parameter headers, String stringParameter, BaseResponseListener listener) {
        synchronized (HttpRequest.class) {
            HttpRequest httpRequest = new HttpRequest();
            httpRequest.context = new WeakReference<Context>(context);
            httpRequest.headers = headers;
...
//                        });
    ...
    private void onDownloadFail(final Exception e) {
        if (DEBUGMODE) {
            LockLog.Builder.create()
                    .e(">>>", "-------------------------------------")
                    .e(">>>", "下载失败:" + e.getMessage())
